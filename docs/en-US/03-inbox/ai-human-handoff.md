...
While AI is handling the conversation, a separate status bar above the composer shows the current Agent and **Take over**. If another Agent is available and you have channel management permission, select the Agent name with the dropdown arrow, choose an Agent in the dialog, and confirm the switch. If no arrow is shown, selecting the name opens that Agent's settings.
