...
Open **Channels**, select the Website Widget, and find **Collect visitor email** in appearance settings, above **Hide Powered by YundaDesk**. It is on by default: when a visitor starts their first chat, the widget asks for an email and they can skip. Turn it off to open the conversation immediately.

You can also ask Yuna to turn visitor email collection on or off for a named Website Widget. Check the selected channel and the proposed setting before confirming. Turning it off does not delete emails already saved, and visitors can still share a contact later in the conversation.
