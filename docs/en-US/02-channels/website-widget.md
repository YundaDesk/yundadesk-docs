...
Each Website channel has its own installation identifier, visitor identity, and conversation history. Previews for multiple Website Widgets in the same workspace do not share conversations. Always copy the installation code generated by the current Website channel instead of reusing another Widget's code on a different site. When a visitor closes and reopens the same Widget, it preserves the current Home or conversation view, reading position, and already loaded brand images instead of loading from a blank screen or the first message again. If AI is still generating a reply, returning Home or reopening the same conversation restores the three-dot reply indicator; another conversation does not inherit it.
...
Visitors never see internal channel-name chips. The home screen shows the single **Contact us** entry only when at least one public contact method is enabled, with equal-sized stacked icons previewing available types. It opens a compact bottom sheet inside the Widget, where the icon, channel type, and trailing arrow use fixed aligned columns; its entrance animation does not flash a window scrollbar. Link items such as Email, Telegram, and LINE open their validated destinations; QR codes and contact images open inside the same sheet without leaving the Widget. The optional email form for identifying a visitor appears only after **Chat with support** is selected for the first conversation and never appears as a contact method; add a separate Email contact method when you want to publish a support address. The settings-page preview reflects all currently enabled, ordered, and newly added contact methods before save. Credentials, channel IDs, asset IDs, and callback addresses are never exposed. Telegram uses the username returned by the Bot API and displays it as `@username` in channel settings; if Telegram is unreachable, you can enter the validated public username manually and save it.
